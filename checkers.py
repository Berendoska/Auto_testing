from sshcheck import ssh_checkout, upload_files, ssh_getout
import yaml
import pytest
...
class TestPositive:
    ...
    def test_add_archive(self, start_time):  # a создали архив
    ...
    def test_check_e_extract(self,  make_files, start_time):  #
        res = list()
    ...
    def test_check_x_files(self,  make_files, start_time):  # only files
        res = list()
        res.append(ssh_checkout(data["ip"], data["user"], data["passwd"], f'cd {data["folder_in"]}; 7z a -t{data["arc_type"]} {data["folder_out"]}/arx2', "Everything is Ok"))
        res.append(ssh_checkout(data["ip"], data["user"], data["passwd"], f'cd {data["folder_out"]}; 7z x arx2.{data["arc_type"]} -o{data["folder_ext"]} -y', "Everything is Ok"))
        for item in make_files:
    ...
    def test_totality(self, start_time):  # t проверка целостности архива
        res = list()
        res.append(ssh_checkout(data["ip"], data["user"], data["passwd"], f'cd {data["folder_in"]}; 7z a -t{data["arc_type"]} {data["folder_out"]}/arx2', "Everything is Ok"))
    ...
    def test_delete(self, start_time):  # d удаление из архива
        res = list()
        res.append(ssh_checkout(data["ip"], data["user"], data["passwd"], f'cd {data["folder_in"]}; 7z a -t{data["arc_type"]} {data["folder_out"]}/arx2', "Everything is Ok"))
